...
class CorrelatedShadowingGenerator(MapGenerator):
    ...
    def mean_at_channel_and_location(self, ind_channel, v_location):
        ...
        if self.m_source_locations is None:
            raise NotImplementedError(
                "Before enabling this functionality, make sure "
        ...
        if self.combine_channels:
            ...
            for ind_source in range(len(self.source_power)):
                total_mean_at_location_watt += dbm_to_watt(
                    self.source_power[ind_source] + self.f_propagation_gain(
                        np.linalg.norm(v_location - np.expand_dims(
                            self.m_source_locations[:, ind_source], 1),
                                       ord=2,
                                       axis=0)))
    ...
    def _generate_map_tensor(self):
        """ 
        Used by MapGenerator.generate_map_tensor()
        ...
        #     m_source_locations = self.m_source_locations
            ...
            self._m_source_locations = self.grid.random_points_in_the_area(
                num_sources).T
        ...
        return t_power_map, np.zeros(shape=(self.grid.num_points_y,
    ...
    def compare_power_and_service_maps_mlsp(self, t_true_map, d_map_estimate,
        ...
        self.plot_map(t_service_map_entropy,
                      m_measurement_loc,
                      m_axes[0, 4],
                      0,
                      1,
