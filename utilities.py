...
def dB_to_natural(array: NDArray[T]) -> NDArray[T]:  # array is power gain
    return (10**(np.divide(array, 10))).astype(array.dtype)
...
def read_conf_file(conf_filepath):
    dict_params = {}
...
def list_are_close(list1, list2):
    ...
    def compare(x, y):
        return tf.math.reduce_all(
            np.core.numeric.isclose(x, y, rtol=1e-6, atol=0, equal_nan=True))
