...
class KrigingEstimator(MapEstimator):
    ...
    def __init__(
        ...
        # self.grid = grid
        # self.metric = metric
        self._f_shadowing_covariance = f_shadowing_covariance
        self.f_mean = f_mean
    ...
    @property
    def f_shadowing_covariance(self):
        return self._f_shadowing_covariance


class BatchKrigingEstimator(KrigingEstimator):
    name_on_figs = "KrigingEstimator"

    def __init__(self, *args, name_on_figs=None, **kwargs):
        super(BatchKrigingEstimator, self).__init__(*args, **kwargs)
    ...
    def estimate_power_one_channel(self, ind_channel, m_measurement_loc,
                                   m_measurements, test_loc):
        """
        Args:
        ...
        if not self.f_mean:
            ...
            mean_pobs = np.reshape(self.f_mean(ind_channel, m_measurement_loc),
                                   (num_measurements, 1))
            ...
            for ind_num_grid_points in range(num_test_loc):
                mean_pnobs[ind_num_grid_points] = self.f_mean(
                    ind_channel,
                    np.reshape((test_loc[ind_num_grid_points]), (3, 1)))
        ...
        for ind_measurement in range(0, num_measurements):
            cov_obs_nobs[ind_measurement, :] = self.f_shadowing_covariance(
                np.linalg.norm(m_location_transposed[ind_measurement] -
                               test_loc,
                               ord=2,
                               axis=1))
        ...
        # [Daniel, 20230909]  I replaced this:
        # cov_nobs_nobs = self.f_shadowing_covariance(
        #     self.grid.get_distance_matrix_test_loc(test_loc))
        # with this:
        m_dist_nobs_nobs = scipy.spatial.distance.cdist(
            test_loc, test_loc, 'euclidean')
        cov_nobs_nobs = self.f_shadowing_covariance(m_dist_nobs_nobs)
        # because it seems that the covariance calculation was assuming that the
        # test locations were on a grid. Howeve, I did not check that the
        # covariance computation is now correct.
    ...
    def estimate_s2s_old(self,
        ...
        if self.min_service_power is not None:
            ...
            t_service_map_ent = empty_array(
                (num_sources, self.grid.num_points_y, self.grid.num_points_x))
        for ind_src in range(num_sources):
            t_power_map_est[ind_src, :, :], t_power_map_var[
                ind_src, :, :] = self.estimate_power_one_channel_old(ind_src)
            if self.min_service_power is not None:
                t_service_map_est[ind_src, :, :], t_service_map_ent[
                    ind_src, :, :] = self.estimate_service_one_gaussian_channel(
    ...
    def estimate_power_one_channel_old(self, ind_channel):
        ...
        if not self.f_mean:
            ...
            for ind_num_grid_points in range(num_grid_points):
                mean_pnobs[ind_num_grid_points] = self.f_mean(
                    ind_channel,
                    np.reshape((m_all_grid_points[0, :, ind_num_grid_points]),
                               (3, 1)))
...
class GudmundsonBatchKrigingEstimator(BatchKrigingEstimator):
    """
    This class implements the batch kriging estimator for the Gudmundson
    correlation model."""
    ...
    def __init__(self,
        ...
        """
        Args:
            `shadowing_std`: scalar, standard deviation of the shadowing
            `shadowing_correlation_dist`: scalar, correlation distance of the shadowing
        """
        super(GudmundsonBatchKrigingEstimator, self).__init__(*args, **kwargs)
        self._shadowing_std = shadowing_std
        self._shadowing_correlation_dist = shadowing_correlation_dist

    @property
    def shadowing_std(self):
        return self._shadowing_std

    @property
    def shadowing_correlation_dist(self):
        return self._shadowing_correlation_dist

    @shadowing_std.setter
    def shadowing_std(self, value):
        self._shadowing_std = value

    @shadowing_correlation_dist.setter
    def shadowing_correlation_dist(self, value):
        self._shadowing_correlation_dist = value

    @property
    def f_shadowing_covariance(self):
        return CorrelatedShadowingGenerator.gudmundson_correlation(
            shadowing_std=self.shadowing_std,
            shadowing_correlation_dist=self.shadowing_correlation_dist)
