...
try:
    ...
    gsim_logger.warning("CVXPY not installed. Some functions will not work.")
import numpy as np
...
class MultikernelEstimator(MapEstimator):
    """
    Arguments:
        'kernel_type' : can be laplacian,  Gaussian, or other types kernels (
        Note: only laplacian and Gaussian kernels are implemented here, see
        kernel_function)
        ...
        'v_kernel_width': vector of length num_kernels with the kernel widths

        If both v_kernel_widths and max_kernel_width is set to None, they are
    ...
    def __init__(self,
                 ...
                 num_kernels=None,
                 reg_par=1e-2,
                 v_kernel_widths=None,
                 **kwargs):
        super(MultikernelEstimator, self).__init__(**kwargs)
        self.kernel_type = kernel_type
        self.reg_par = reg_par
        assert (num_kernels is None) ^ (v_kernel_widths is None)
        if v_kernel_widths is not None:
            assert max_kernel_width is None
        self._num_kernels = num_kernels
        self.v_kernel_widths = v_kernel_widths
        self.max_kernel_width = max_kernel_width

    @property
    def num_kernels(self):
        return self._num_kernels if self.v_kernel_widths is None else len(
            self.v_kernel_widths)

    # getter for log_reg_par
    @property
    def log_reg_par(self):
        return np.log10(self.reg_par)

    # setter for log_reg_par
    @log_reg_par.setter
    def log_reg_par(self, value):
        self.reg_par = 10.0**(value)
    ...
    def estimate_power_one_channel(self,
                                   ind_channel,
                                   measurement_loc=None,
                                   measurements=None,
                                   test_loc=None):
    ...
    def _estimate_power_map_one_channel_at_loc(self, m_measurement_loc,
        ...
        if self.v_kernel_widths is None:
            if self.max_kernel_width is None:
                m_distances = euclidean_distances(m_measurement_loc,
                                                  m_measurement_loc)
            ...
            if self.kernel_type == "laplacian":
                self.v_kernel_widths = self.max_kernel_width * np.linspace(
                    0.1, 1, self.num_kernels)
            else:
                self.v_kernel_widths = self.max_kernel_width * np.linspace(
                    0.005, 1, self.num_kernels)

        # obtain the kernel coefficients
        t_kernel_matrices = np.zeros(
            (num_mea_loc, num_mea_loc, self.num_kernels))
        for ind_loc in range(num_mea_loc):
            ...
            for ind_kernel in range(self.num_kernels):
                t_kernel_matrices[ind_loc, :, ind_kernel] = kernel_function(
                    m_row_inputs_to_kernel_func,
                    m_measurement_loc,
                    self.v_kernel_widths[ind_kernel],
                    kernel_type=self.kernel_type)

        m_all_kernel_matrices = np.reshape(
            t_kernel_matrices, (num_mea_loc, num_mea_loc * self.num_kernels),
            order='F')
        t_chol_matrices = np.zeros(t_kernel_matrices.shape)
        for ind_kernel in range(self.num_kernels):
            t_chol_matrices[:, :, ind_kernel] = np.linalg.cholesky(
                t_kernel_matrices[:, :, ind_kernel] + 1e-7 *
                np.eye(num_mea_loc))  # or 1e-7 worked better with laplacian

        v_kernel_coeffs = cpy.Variable(self.num_kernels * num_mea_loc)
        m_all_chol_matrices = np.reshape(
            t_chol_matrices, (num_mea_loc, num_mea_loc * self.num_kernels),
            order='F')
        gamma = cpy.Parameter(nonneg=True)

        objective = cpy.Minimize(
            0.5 * cpy.sum_squares(
                (m_all_kernel_matrices @ v_kernel_coeffs) - v_measurements) +
            gamma *
            (cpy.norm2(cpy.sum_squares(m_all_chol_matrices @ v_kernel_coeffs)))
        )
        ...
        for ind_loc in range(num_test_loc):
            ...
            for ind_kernel in range(self.num_kernels):
                v_kernel_vecs[:, ind_kernel] = kernel_function(
                    m_query_point_rep,
                    m_measurement_loc,
                    self.v_kernel_widths[ind_kernel],
                    kernel_type=self.kernel_type)
                v_power_map_est_one_ch[ind_loc, 0] = (
                    v_kernel_vecs.flatten('F').dot(v_kernel_coeffs))
...
class KernelRidgeRegressionEstimator(MapEstimator):
    ...
    def _estimate_power_map_one_channel_at_loc(self, m_measurement_loc,
        ...
        for ind_loc in range(num_mea_loc):
            v_point_1 = m_measurement_loc[ind_loc, :]
            m_row_inputs_to_kernel_func = np.tile(v_point_1, (num_mea_loc, 1))
            m_kernel_matrix[ind_loc] = kernel_function(
                m_row_inputs_to_kernel_func,
                m_measurement_loc,
                kernel_width=self.kernel_width,
                kernel_type=self.kernel_type)
        v_kernel_coeff = np.linalg.inv(m_kernel_matrix +
                                       num_mea_loc * self.reg_par *
                                       np.eye(num_mea_loc)).dot(v_measurements)
        ...
        for ind_loc in range(num_test_loc):
            v_query_point = m_test_loc[ind_loc, :]
            m_query_point_rep = np.tile(v_query_point, (num_mea_loc, 1))
            v_kernel_vec = kernel_function(m_query_point_rep,
                                           m_measurement_loc,
                                           kernel_width=self.kernel_width,
                                           kernel_type=self.kernel_type)
            v_power_map_est_one_ch[ind_loc,
                                   0] = v_kernel_vec.dot(v_kernel_coeff)
        return v_power_map_est_one_ch
...
class KNNEstimator(MapEstimator):
    ...
    def _estimate_power_map_one_channel_at_loc(self, m_measurement_loc,
        ...
        for ind_loc in range(num_test_loc):
            ...
            v_dist_to_all_meas = np.linalg.norm(np.subtract(
                m_current_point_rep, m_measurement_loc),
                                                axis=1)
            v_neighbor_meas = v_measurements[np.argsort(v_dist_to_all_meas)
                                             [0:self.num_neighbors]]
...
def kernel_function(x1, x2, kernel_width, kernel_type):
    if kernel_type == "laplacian":
        output = np.exp(-np.linalg.norm(np.subtract(x1, x2), axis=1) /
                        kernel_width)
    elif kernel_type == "gaussian":
        output = np.exp(-np.linalg.norm(np.subtract(x1, x2), axis=1)**2 /
                        (kernel_width**2))
    else:
