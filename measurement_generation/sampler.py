...
class InterpolationSampler(Sampler):
    ...
    def sampled_map_multiple_grid_loc(self,
                                      t_true_map,
                                      num_measurements=1,
        ...
        """
        t_sample_mask = np.zeros((1, t_true_map.shape[1], t_true_map.shape[2]))

        v_indices_to_sampled_from = self.grid.random_grid_points_inds_outside_buildings(
    ...
    def sample_map(self, t_map, v_sample_location):
        ...
        if self.interpolation_method == "avg_nearest":
            ...
            min_dist_index = self.grid.nearest_gridpoint_inds(
                v_sample_location)
...
################### old method ########################
    ...
    def sampled_map_multiple_grid_loc_old(self, t_true_map,
                                          m_sample_locations):
        """
        Args:
            `m_sample_locations`: num_points x 3 matrix with 3D locations of
        ...
        for v_sample_location in m_sample_locations:
            v_measurement = self.sample_map(
                t_map=t_true_map, v_sample_location=v_sample_location)
            'get the grid point indices'
            v_measurement_loc_inds = self.grid.nearest_gridpoint_inds(
                v_sample_location)

            for ind_sources in range(num_sources):
                t_all_measurements_grid[
                    ind_sources, v_measurement_loc_inds[0],
                    v_measurement_loc_inds[1]] = v_measurement[ind_sources]
            t_sample_mask[0, v_measurement_loc_inds[0],
